...
$url = new moodle_url('/mod/data/field/latlonggps/kml.php', array('d'=>$d, 'fieldid'=>$fieldid));
if ($rid !== 0) {
    $url->param('rid', $rid);
}
$PAGE->set_url($url);
...
if ($rid) {
    ...
    if (! $field->type == 'latlonggps') { // Make sure we're looking at a latlonggps data type!
...
echo data_latlonggps_kml_top();

if($rid) { // List one single item
    $pm = new stdClass();
    $pm->name = data_latlonggps_kml_get_item_name($content, $field);
    ...
    echo data_latlonggps_kml_placemark($pm);
...
echo data_latlonggps_kml_bottom();




function data_latlonggps_kml_top() {
...
function data_latlonggps_kml_placemark($pm) {
    return '<Placemark>
...
function data_latlonggps_kml_bottom() {
    return '</kml>';
}

function data_latlonggps_kml_get_item_name($content, $field) {
    global $DB;
